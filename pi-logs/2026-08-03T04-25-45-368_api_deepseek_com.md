...
- **timestamp**: 2026-08-03T04:25:45.368Z
...
- **tools**: 20 definitions, 26,413 bytes (~6,603 tokens)
- **system prompt**: 13,839 bytes (~3,460 tokens)
- **total request**: 40,628 bytes

**Tools, ranked by size:**

| tool | bytes | ~tokens | % of request |
| --- | --: | --: | --: |
| Agent | 7,807 | ~1,952 | 19.2% |
| ask_user_question | 3,815 | ~954 | 9.4% |
| mcp | 2,550 | ~638 | 6.3% |
| fetch_url | 1,215 | ~304 | 3.0% |
| edit | 1,194 | ~299 | 2.9% |
| grep | 1,054 | ~264 | 2.6% |
| hypa_grep | 930 | ~233 | 2.3% |
| web_search | 757 | ~189 | 1.9% |
| read | 699 | ~175 | 1.7% |
| hypa_read | 683 | ~171 | 1.7% |
| find | 631 | ~158 | 1.6% |
| steer_subagent | 627 | ~157 | 1.5% |
| hypa_find | 606 | ~152 | 1.5% |
| get_subagent_result | 597 | ~149 | 1.5% |
| gemini_video | 588 | ~147 | 1.4% |
| hypa_ls | 586 | ~147 | 1.4% |
| hypa_shell | 585 | ~146 | 1.4% |
| bash | 557 | ~139 | 1.4% |
| ls | 487 | ~122 | 1.2% |
| write | 445 | ~111 | 1.1% |
...
x-stainless-runtime-version: v26.5.0
...
- Main documentation: /home/hasar/.cache/aube/virtual-store/@earendil-works+pi-coding-agent@0.82.1-7a288f34a3935d4d/node_modules/@earendil-works/pi-coding-agent/README.md
- Additional docs: /home/hasar/.cache/aube/virtual-store/@earendil-works+pi-coding-agent@0.82.1-7a288f34a3935d4d/node_modules/@earendil-works/pi-coding-agent/docs
- Examples: /home/hasar/.cache/aube/virtual-store/@earendil-works+pi-coding-agent@0.82.1-7a288f34a3935d4d/node_modules/@earendil-works/pi-coding-agent/examples (extensions, custom tools, SDK)
...
<project_instructions path="/home/hasar/.pi/agent/AGENTS.md">
...
<project_instructions path="/home/hasar/CLAUDE.md">
You're Gator the Delegator. You hold one problem in full resolution, delegate everything supplementary, and give concise, visually rich, highly scannable responses.
...
Current working directory: /home/hasar/projects/dotfiles
...
Read a web page or PDF. Handles JS-rendered pages, Cloudflare and anti-bot walls. Pass `prompt` with what you need from the page — a model reads the full page and returns just that, which is far cheaper than reading the page yourself. Omit `prompt` only when you need the page text itself.
...
```json
{
  ...
  "properties": {
    "url": {
      "type": "string",
      "description": "http/https URL of a page or PDF"
    },
    "prompt": {
      "type": "string",
      "description": "What to extract, as a question or instruction. Be specific — it drives both the extraction and the relevance ranking."
    },
    "raw": {
      "type": "boolean",
      "description": "Return page text instead of a model's answer. With `prompt`, returns the BM25-relevant blocks for it. Costs nothing and needs no API key."
    },
    "max_tokens": {
      "type": "number",
      "description": "Ceiling on returned page text in raw mode (default 1200)"
    },
    "offset": {
      "type": "number",
    ...
    "pages": {
      "type": "string",
      "description": "PDF only: page spec like '1-5' or '2,7-9'"
    },
    "fresh": {
      "type": "boolean",
      "description": "Bypass hound's cache and refetch"
    }
  }
}
```
...
Analyze a video with Gemini: summarize, transcribe, answer questions, or extract timestamps. `source` is a public YouTube URL or a local video file path.
...
```json
{
  ...
  "properties": {
    "source": {
      "type": "string",
      "description": "Public YouTube URL or local video file path"
    },
    "prompt": {
      "type": "string",
      "description": "What to do with the video, e.g. 'summarize with timestamps'"
    },
    "model": {
      "type": "string",
...
Run shell commands through Hypa compression. Output is truncated to 2000 lines or 50KB with full output saved when needed.
...
```json
{
  ...
  "properties": {
    "command": {
      "type": "string",
      "description": "Shell command to execute through Hypa compression"
    },
    "timeoutMs": {
      "type": "number",
      "description": "Timeout in milliseconds (default: Hypa CLI default)"
    },
    "raw": {
      "type": "boolean",
      "description": "Run with hypa raw instead of compressed hypa -c"
    }
  },
  "required": [
    "command"
...
Read a file through Hypa compression. Supports offset/limit line slices. Output is truncated to 2000 lines or 50KB with full output saved when needed.
...
```json
{
  ...
  "properties": {
    "path": {
      "type": "string",
      "description": "Path to read, relative to Pi cwd or absolute"
    },
    "offset": {
      "type": "number",
      "description": "Line number to start reading from (1-indexed)"
    ...
    "maxTokens": {
      "type": "number",
      "description": "Approximate maximum tokens to return after Hypa compression"
    }
  },
  "required": [
    "path"
...
Search file contents with ripgrep through Hypa compression. Output is truncated to 2000 lines or 50KB with full output saved when needed.
...
Find files through Hypa compression. Output is truncated to 2000 lines or 50KB with full output saved when needed.
...
```json
{
  ...
  "properties": {
    "pattern": {
      "type": "string",
      "description": "File name/glob pattern (default: *)"
    },
    "path": {
      "type": "string",
...
List directory contents through Hypa compression. Output is truncated to 2000 lines or 50KB with full output saved when needed.
...
Servers: serena (22 tools), context-mode (11 tools), deepwiki (3 tools), hound (6 tools)
...
- Explore: Fast read-only search agent for locating code. Use it to find files by pattern (eg. "src/components/**/*.tsx"), grep for symbols or keywords (eg. "API endpoints"), or answer "where is X defined / which files reference Y." Do NOT use it for code review, design-doc auditing, cross-file consistency checks, or open-ended analysis — it reads excerpts rather than whole files and will miss content past its read window. When calling, specify search breadth: "quick" for a single targeted lookup, "medium" for moderate exploration, or "very thorough" to search across multiple locations and naming conventions. (claude-haiku-4-5) (Tools: read, bash, grep, find, ls)
- Plan: Software architect agent for designing implementation plans. Use this when you need to plan the implementation strategy for a task. Returns step-by-step plans, identifies critical files, and considers architectural trade-offs. (Tools: read, bash, grep, find, ls)

Custom agents can be defined in .pi/agents/<name>.md (project) or /home/hasar/.pi/agent/agents/<name>.md (global) — they are picked up automatically. Project-level agents override global ones. Creating a .md file with the same name as a default agent overrides it.
...
```json
{
  ...
  "properties": {
    ...
    "subagent_type": {
      "type": "string",
      "description": "The type of specialized agent to use. Available types: general-purpose, Explore, Plan. Custom agents from .pi/agents/*.md (project) or /home/hasar/.pi/agent/agents/*.md (global) are also available."
    },
    "model": {
      "type": "string",
      ...
      "items": {
        ...
        "properties": {
          ...
          "header": {
            "type": "string",
            "maxLength": 16,
            "description": "MAX 16 CHARACTERS — hard limit, requests over the limit are rejected. Very short chip/tag shown next to the question. Examples: \"Auth method\", \"Library\", \"Approach\"."
          },
          "options": {
            "type": "array",
            "items": {
              ...
              "properties": {
                "label": {
                  "type": "string",
                  "maxLength": 60,
                  "description": "MAX 60 CHARACTERS — hard limit, requests over the limit are rejected. The display text for this option that the user will see and select. Should be concise (1-5 words) and clearly describe the choice."
                },
                "description": {
                  "type": "string",
...
Search the web via DeepSeek. Returns search results with titles, URLs, and a brief summary. The current date is August 2026 — use the current year for recent queries.
