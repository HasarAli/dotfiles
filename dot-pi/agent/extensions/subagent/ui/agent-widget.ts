...
export class AgentWidget {
  ...
  private renderFinishedLine(a: { id: string; status: string; description: string; toolUses: number; startedAt: number; completedAt?: number; error?: string }, theme: Theme): string {
    ...
    return `${icon} ${theme.fg("dim", "Agent")}  ${theme.fg("dim", a.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", parts.join(" · "))}${statusText}`;
  }
  ...
  private renderWidget(tui: any, theme: Theme): string[] {
    ...
    for (const a of running) {
      ...
      runningLines.push([
        truncate(theme.fg("dim", "├─") + ` ${theme.fg("accent", frame)} ${theme.bold("Agent")}  ${theme.fg("muted", a.description)} ${theme.fg("dim", "·")} ${fgPreservingNestedStyles(theme, "dim", statsText)}`),
